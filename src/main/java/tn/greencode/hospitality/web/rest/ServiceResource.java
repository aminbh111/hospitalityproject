...
/**
 * REST controller for managing {@link tn.greencode.hospitality.domain.Service}.
 */
@RestController
@RequestMapping("/api")
@Transactional
        ...
        Optional<Service> result = serviceRepository
            ...
            .map(existingService -> {
                if (service.getDate() != null) {
                    existingService.setDate(service.getDate());
                }
                if (service.getPublish() != null) {
                    existingService.setPublish(service.getPublish());
                }
                if (service.getContentPosition() != null) {
                    existingService.setContentPosition(service.getContentPosition());
                }
                if (service.getImagePosition() != null) {
                    existingService.setImagePosition(service.getImagePosition());
                }
