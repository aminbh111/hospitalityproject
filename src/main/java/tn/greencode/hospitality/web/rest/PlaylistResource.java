...
import tn.greencode.hospitality.repository.PlaylistRepository;
import tn.greencode.hospitality.web.rest.errors.BadRequestAlertException;

/**
 * REST controller for managing {@link tn.greencode.hospitality.domain.Playlist}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class PlaylistResource {
        ...
        Optional<Playlist> result = playlistRepository
            ...
            .map(existingPlaylist -> {
                if (playlist.getDate() != null) {
                    existingPlaylist.setDate(playlist.getDate());
                }
                if (playlist.getLang() != null) {
                    existingPlaylist.setLang(playlist.getLang());
                }
                if (playlist.getFile() != null) {
                    existingPlaylist.setFile(playlist.getFile());
                }
    ...
    public ResponseEntity<List<Playlist>> getAllPlaylists(@org.springdoc.api.annotations.ParameterObject Pageable pageable) {
